...
# The interface intentionally resembles an engineering workstation rather than Streamlit's default application appearance.
st.html(
    """
    <style>
...
# Persistent Streamlit state keeps expensive FastF1 work available while the user navigates between dashboard sections.
...
# Driver changes are inexpensive compared with loading the whole FastF1 session, so rebuild only when the selected pair changes
